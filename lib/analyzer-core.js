import {decode2920,powers} from './did2920.js';
...
export function profileRows(profile){
 ...
 return profile.dids.map(d=>{
  ...
  if(d.command!==r.cmd||seen.has(r.did))throw Error('command不一致またはDID重複');
  seen.add(r.did);return {...r,enabled:d.enabled!==false,status:d.status||'unknown_candidate'};
 });
}
...
export class Transport{
 ...
 context(){const p=this.pending||this.retired;return p?{seq:p.seq,cmd:p.cmd,...(p.did?{did:p.did}:{})}:{seq:null,cmd:null};}
 ...
 async request(cmd,timeout=3000){
  ...
  const p={seq:++this.seq,cmd,did:cmd.startsWith('22')?cmd.slice(2):undefined};
