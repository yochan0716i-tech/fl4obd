import {VERSION,Transport,initialize,sleep,row,profileRows,nextDid} from './analyzer-core.js';
...
function emit(data){
 ...
 if(data.type==='rx'&&data.did&&!data.late&&!data.unsolicited){
  const r=rows.find(r=>r.did===data.did);const k=data.ok?'countOk':'countErr';
  if(r){r[k]++;r.lastResponse=event.ts;}
  if(recording||stopping){const count=totals.get(data.did)||{ok:0,error:0};count[data.ok?'ok':'error']++;totals.set(data.did,count);}
 }
...
function config(){return rows.map(({did,cmd,enabled,intervalMs})=>({did,cmd,enabled,interval_ms:intervalMs}));}
function refresh(){
 ...
 for(const r of rows){const tr=document.querySelector('[data-did="'+r.did+'"]');if(tr){tr.querySelector('.ok').textContent=r.countOk;tr.querySelector('.err').textContent=r.countErr;tr.querySelector('.last').textContent=r.lastResponse?.slice(11,23)||'—';}}
}
function table(){
 $('dids').replaceChildren();
 for(const r of rows){
  const tr=document.createElement('tr');tr.dataset.did=r.did;
  const enabled=document.createElement('input');enabled.type='checkbox';enabled.checked=r.enabled;enabled.setAttribute('aria-label',r.did+' 有効');
  enabled.onchange=()=>{r.enabled=enabled.checked;r.nextDue=performance.now();emit({type:'config',dids:config()});};
  const interval=document.createElement('input');interval.type='number';interval.min=100;interval.max=3600000;interval.step=100;interval.value=r.intervalMs;interval.setAttribute('aria-label',r.did+' 周期ms');
  ...
  interval.onchange=()=>{try{const checked=row(r.did,Number(interval.value));r.intervalMs=checked.intervalMs;r.nextDue=performance.now();emit({type:'config',dids:config()});}catch(e){message(e.message);interval.value=r.intervalMs;}};
  const remove=document.createElement('button');remove.textContent='削除';remove.setAttribute('aria-label',r.did+' 削除');remove.onclick=()=>{rows=rows.filter(x=>x!==r);emit({type:'config',dids:config()});table();};
  for(const value of [enabled,r.did,r.cmd,interval,r.status||'unknown_candidate']){const td=document.createElement('td');if(value instanceof Node)td.append(value);else td.textContent=value;tr.append(td);}
...
$('mark').onclick=()=>{if(recording)emit({type:'mark',label:$('marker').value,note:$('note').value});};
...
try{
 ...
 for(const label of profile.markers||['CUSTOM']){const option=document.createElement('option');option.value=label;option.textContent=label;$('marker').append(option);}
