import {VERSION,Transport,initialize,sleep,row,pidRow,profileRows,nextDid} from './analyzer-core.js?v=20260920g';
import {selectExportEvents,toJsonl} from './analyzer-export.js?v=20260920g';
...
let elm='',protocol='',state='未接続',wake=null,db=null,events=[],writes=Promise.resolve(),persistenceError='',clearing=false;
...
function refresh(){
 ...
 $('start').disabled=recording||stopping||clearing||!profile;
 $('clearLogs').disabled=recording||stopping||clearing;
 $('save').disabled=clearing;$('saveFull').disabled=clearing;$('stop').disabled=!recording;
...
function startRecording(){
 if(recording||stopping||clearing||!profile)return;
...
async function clearLogs(){
 if(recording||stopping||clearing)return;
 if(!confirm('端末内の全ログを削除します。未保存のログも失われます。削除しますか？'))return;
 clearing=true;refresh();
 // Queue deletion after pending writes. Events arriving after this boundary are retained.
 const count=events.length,items=[...$('recent').children];
 const operation=writes.then(()=>new Promise((resolve,reject)=>{
  if(!db){reject(Error('IndexedDBを利用できません'));return;}
  const tx=db.transaction('events','readwrite');tx.objectStore('events').clear();
  tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error||Error('ログ削除失敗'));
  tx.onabort=()=>reject(tx.error||Error('ログ削除中断'));
 }));
 // Keep the journal queue usable even if deletion fails.
 writes=operation.catch(()=>{});
 try{
  await operation;
  events.splice(0,count);for(const item of items)item.remove();
  // Preserve seq: the current transport may still refer to an in-flight request.
  persistenceError='';
  message('端末内ログを削除しました');
 }catch(e){message('端末内ログを削除できませんでした: '+e.message);}
